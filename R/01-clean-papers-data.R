...
for(i in 1: length(unique(utm$verbatimLatitude))) {
  x <- utm_data_modif %>% filter(verbatimLatitude == sort(unique(utm$verbatimLatitude))[i]) %>% select(verbatimLatitude, verbatimLongitude, decimalLatitude, decimalLongitude)
  df <- bind_rows(x[1,], df)
}
