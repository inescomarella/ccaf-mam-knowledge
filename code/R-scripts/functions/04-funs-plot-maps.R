...
count.orders.regs.in.polygons <- function(pts, polygons, order_list) {
  # Count number of records of each order in each polygon
  # Args:
  #   pts: sf point object containing a "order" columns
  #   polygons: sf multipolygon object
  #   order_list: list of characters containting order names
  # Output:
  ...
  colnames(polygons)[(ncol(polygons) - length(order_list) + 1):ncol(polygons)] <-
    order_list
  return(polygons)
}
