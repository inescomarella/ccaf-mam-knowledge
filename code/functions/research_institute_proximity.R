research_institute_proximity <- function(grid, institutes_points) {
  ...
  for (i in 1:length(institutes)) {
    for (j in 1:length(grid_ids)) {
      ...
      pri_df[j, i] <- 10^8 * rel_contrib_j / as.numeric(dist_ij)
    }
  }
  ...
  pri_impact <- pri_df %>%
    ...
    mutate(proximity = sum(c_across(-c(grid_id)), na.rm = TRUE)) %>%
    select(grid_id, proximity)
